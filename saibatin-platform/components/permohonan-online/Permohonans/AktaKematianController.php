...
{   
    ...
    {   
        $pelayananID    = '10'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.aktakematian', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    {   
        ...
        if($recaptchaRes->isSuccess()):
            ...
            $nikjenazah = $request->get('nikjenazah');
            ...
            $tglkematian    = $request->get('tglkematian');
            $jamkematian    = $request->get('jamkematian');
            $tempatkematian     = $request->get('tempatkematian');
            $sebabkematian  = $request->get('sebabkematian');
            $menerangkankematian    = $request->get('menerangkankematian');
            ...
            $fileketkematianx   = $request->get('fileketkematianx');
            $filekkx    = $request->get('filekkx');
            $filektppelaporx    = $request->get('filektppelaporx');
            $filektpjenazahx    = $request->get('filektpjenazahx');
            ...
            if($act=='ins'):
                ...
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'anakke'    => 'required',
                        'tglkematian'   => 'required',
                        'jamkematian'   => 'required',
                        'tempatkematian'=> 'required',
                        'sebabkematian' => 'required',
                        'menerangkankematian'  => 'required',
                        'niksaksi1' => 'required',
                        'namasaksi1'=> 'required',
                        'niksaksi2' => 'required',
                        'namasaksi2'=> 'required',
                        'fileketkematianx'   => 'required',
                        'filekkx'    => 'required',
                        'filektppelaporx'    => 'required',
                        'filektpjenazahx'    => 'required'
                    ]
                    ,[
                        'required' 	=> ':attribute'
                    ]
    	        ...
    	        if($validation->fails()):
    	            ...
    	            if($sessnik):
    	                if($act == "ins"):
    	                	try {
    	                        ...
    	                        $dataprepcv = new KematianModel([
                                    ...
                                    'biodata_jenazahNik'=> $nikjenazah,
    		                        'biodata_jenazahNama'	=> $namalengkap,
    								'biodata_jenazahJenisKelamin'  => $jeniskelamin,
    								'biodata_jenazahTanggalLahir' 	=> $tgllahir,
    								...
    								'dataKematian_anakKe' 	=> $anakke,
    								'dataKematian_tanggalKematian'	=> $tglkematian,
    								'dataKematian_jamKematian'  	=> $jamkematian,
    								'dataKematian_tmptKematian'	=> $tempatkematian,
    								'dataKematian_sebabKematian' 	=> $sebabkematian,
    								'dataKematian_menerangkanKematian'   => $menerangkankematian,
    								'saksi1_nik'=> $niksaksi1,
    								'saksi1_nama'	=> $namasaksi1,
    								'saksi2_nik'=> $niksaksi2,
    								'saksi2_nama' 	=> $namasaksi2,
    								'catatan_detail'=> $catatan,
                                    'syaratDok_keteranganKematian'   => isset($fileketkematianx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileketkematianx:'',
                                    'syaratDok_KK'  => isset($filekkx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkx:'',
                                    'syaratDok_KTPPelapor'  => isset($filektppelaporx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filektppelaporx:'',
                                    'syaratDok_KTPJenazah'  => isset($filektpjenazahx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filektpjenazahx:'',
                                ...
                                #whereIn('progress_status', ['2','3'])->where([['biodata_ayahNama','=',$namaayah],['biodata_jenazahNama','=',$namalengkap],['status','=','1']]);
    	                        $isExistQuery= KematianModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
    	                        if($isExistQuery->count()<=0):
    		                        ...
    		                        if($savedata):
                                        ...
                                        $fileList  = ['fileketkematianx','filekkx','filektppelaporx','filektpjenazahx','filependukung1x','filependukung2x'];
                                        ...
                                        $PermohonanData     = KematianModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
    	                ...
    	                elseif($request->get('act') == "upd"):
                            try {
                                $data = KematianModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new KematianModel;
                                        ...
                                        $datamaskingMass= KematianModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KematianModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();

        #$getdatas   = file_get_contents('http://36.92.154.132/app.core.002/getdataI/'.$data->biodata_jenazahNik.'/'.$data->biodata_Kk);
        #$getdatas   = json_decode($getdatas);
        #$getdatas   = $getdatas->data[0];
        ...
        $output = array(
            ...
            ,'nikjenazah'   => $data->biodata_jenazahNik
            ,'namalengkap'  => $data->biodata_jenazahNama #$getdatas->namalengkap
            ,'jeniskelamin' => $data->biodata_jenazahJenisKelamin #$getdatas->jenkel
            ,'tgllahir'     => $data->biodata_jenazahTanggalLahir #$getdatas->tgllhr
            ,'nikayah'     => $data->biodata_ayahNik
            ,'namaayah'     => $data->biodata_ayahNama #$getdatas->ayahnama
            ,'pekerjaan'    => $data->biodata_ayahPekerjaan #$getdatas->pekerjaan
            ,'namaibu'  => $data->biodata_ibuNik
            ,'nikibu'  => $data->biodata_ibuNama #$getdatas->ibunama
            ,'anakke'   => $data->dataKematian_anakKe
            ,'tglkematian'  => $data->dataKematian_tanggalKematian
            ,'jamkematian'  => $data->dataKematian_jamKematian
            ,'tempatkematian'   => $data->dataKematian_tmptKematian
            ,'sebabkematian'=> $data->dataKematian_sebabKematian
            ,'menerangkankematian'  => $data->dataKematian_menerangkanKematian
            ...
            ,'fileketkematian'  => $data->syaratDok_keteranganKematian
            ,'filekk' => $data->syaratDok_KK
            ,'filektppelapor' => $data->syaratDok_KTPPelapor
            ,'filektpjenazah' => $data->syaratDok_KTPJenazah
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KematianModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $data_jenkel  = OptionModel::where([['option_val','=',$data->biodata_jenazahJenisKelamin],['option_group','=','jenkel'],['status','=','1']])->get()->first();
        $data_kematianSebab = OptionModel::where([['option_val','=',$data->dataKematian_sebabKematian],['option_group','=','kematianSebab'],['status','=','1']])->get()->first();
        $data_kematianMenerangkan = OptionModel::where([['option_val','=',$data->dataKematian_menerangkanKematian],['option_group','=','kematianMenerangkan'],['status','=','1']])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => strtolower($data->pemohon_nama)
            ,'pemohonkk'    => $data->pemohon_kk
            ,'pemohonhp'    => $data->pemohon_hp
            ,'pemohonemail' => strtolower($data->pemohon_email)
            ,'nikjenazah'   => $data->biodata_jenazahNik
            ,'namalengkap'  => strtolower($data->biodata_jenazahNama) #$getdatas->namalengkap
            ,'jeniskelamin' => strtolower($data_jenkel->option_name) #$getdatas->jenkel
            ,'tgllahir'     => $data->biodata_jenazahTanggalLahir #$getdatas->tgllhr
            ,'nikayah'      => $data->biodata_ayahNik
            ,'namaayah'     => strtolower($data->biodata_ayahNama) #$getdatas->ayahnama
            ,'pekerjaan'    => strtolower($data->biodata_ayahPekerjaan) #$getdatas->pekerjaan
            ,'namaibu'  => strtolower($data->biodata_ibuNik)
            ,'nikibu'   => $data->biodata_ibuNama #$getdatas->ibunama

            ,'anakke'   => $data->dataKematian_anakKe
            ,'tglkematian'  => $data->dataKematian_tanggalKematian
            ,'jamkematian'  => $data->dataKematian_jamKematian
            ,'tempatkematian'   => strtolower($data->dataKematian_tmptKematian)
            ,'sebabkematian'=> strtolower($data_kematianSebab->option_name)
            ,'menerangkankematian'  => strtolower($data_kematianMenerangkan->option_name)

            ,'niksaksi1'   => $data->saksi1_nik
            ,'namasaksi1'  => strtolower($data->saksi1_nama)
            ,'niksaksi2'   => $data->saksi2_nik
            ,'namasaksi2'  => strtolower($data->saksi2_nama)
            ,'catatan'   => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'fileketkematian'  => ($data->syaratDok_keteranganKematian)?'ADA':'-'
            ,'filekk' => ($data->syaratDok_KK)?'ADA':'-'
            ,'filektppelapor' => ($data->syaratDok_KTPPelapor)?'ADA':'-'
            ,'filektpjenazah' => ($data->syaratDok_KTPJenazah)?'ADA':'-'
        ...
        $pdf = \PDF::loadView('fronts.permohonans.aktakematianPDF', $output);
