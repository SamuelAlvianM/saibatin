...
{   
    ...
    {   
        $pelayananID    = '11'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.aktanikah', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            $nokksuami  = $request->get('nokksuami');
            $niksuami   = $request->get('niksuami');
            $suamianakke    = $request->get('suamianakke');
            $nokkistri  = $request->get('nokkistri');
            $nikistri   = $request->get('nikistri');
            $istrianakke    = $request->get('istrianakke');
            $niksaksi1  = $request->get('niksaksi1');
            $niksaksi2  = $request->get('niksaksi2');
            $tglpemberkatan     = $request->get('tglpemberkatan');
            $tmptpemberkatan    = $request->get('tmptpemberkatan');
            $filekksuamix   = $request->get('filekksuamix');
            $filektpsuamix  = $request->get('filektpsuamix');
            $filekkistrix   = $request->get('filekkistrix');
            $filektpistrix  = $request->get('filektpistrix');
            $filesuamiistrix    = $request->get('filesuamiistrix');
            $filebukunikahagamax    = $request->get('filebukunikahagamax');
            ...
            if($act=='ins'):
                ...
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'nokksuami'     => 'required',
                        'niksuami'      => 'required',
                        'suamianakke'   => 'required',
                        'nokkistri'     => 'required',
                        'nikistri'      => 'required',
                        'istrianakke'   => 'required',
                        'niksaksi1'     => 'required',
                        'niksaksi2'     => 'required',
                        'tglpemberkatan'    => 'required',
                        'tmptpemberkatan'   => 'required',
                        
                        'filekksuamix' => 'required',
                        'filektpsuamix'    => 'required',
                        'filekkistrix'   => 'required',
                        'filektpistrix'   => 'required',
                        'filesuamiistrix'   => 'required',
                        'filebukunikahagamax'   => 'required'
                    ]
                    ,[
                        'required'  => ':attribute'
                    ]
                    ...
                    if($sessnik):
                        if($act == "ins"):
                            try {
                                ...
                                $dataprepcv = new PerkawinanModel([
                                    ...
                                    'aktaPerkawinan_nokksuami'  => $nokksuami,
                                    'aktaPerkawinan_niksuami'   => $niksuami,
                                    'aktaPerkawinan_suamianakke'=> $suamianakke,
                                    'aktaPerkawinan_nokkistri'  => $nokkistri,
                                    'aktaPerkawinan_nikistri'   => $nikistri,
                                    'aktaPerkawinan_istrianakke'    => $istrianakke,
                                    'aktaPerkawinan_niksaksi1'  => $niksaksi1,
                                    'aktaPerkawinan_niksaksi2'  => $niksaksi2,
                                    'aktaPerkawinan_tglpemberkatan' => $tglpemberkatan,
                                    'aktaPerkawinan_tmptpemberkatan'=> $tmptpemberkatan,
                                    'catatan_detail'=> $catatan,
                                    'syaratDok_KKSuami'     => isset($filekksuamix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekksuamix:'',
                                    'syaratDok_KTPSuami'    => isset($filektpsuamix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filektpsuamix:'',
                                    'syaratDok_KKIstri'     => isset($filekkistrix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkistrix:'',
                                    'syaratDok_KTPIstri'    => isset($filektpistrix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filektpistrix:'',
                                    'syaratDok_FotoSuamiIstri' => isset($filesuamiistrix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filesuamiistrix:'',
                                    'syaratDok_BukuNikahAgama' => isset($filebukunikahagamax)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filebukunikahagamax:'',
                                ...
                                $isExistQuery= PerkawinanModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filekksuamix','filektpsuamix','filekkistrix','filektpistrix','filesuamiistrix','filebukunikahagamax','filependukung1x','filependukung2x'];
                                        ...
                                        $PermohonanData     = PerkawinanModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = PerkawinanModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new PerkawinanModel;
                                        ...
                                        $datamaskingMass= PerkawinanModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = PerkawinanModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        ...
        $output = array(
            ...
            ,'nokksuami'    => $data->aktaPerkawinan_nokksuami
            ,'niksuami'     => $data->aktaPerkawinan_niksuami
            ,'suamianakke'  => $data->aktaPerkawinan_suamianakke
            ,'nokkistri'    => $data->aktaPerkawinan_nokkistri
            ,'nikistri'     => $data->aktaPerkawinan_nikistri
            ,'istrianakke'      => $data->aktaPerkawinan_istrianakke
            ,'niksaksi1'    => $data->aktaPerkawinan_niksaksi1
            ,'niksaksi2'    => $data->aktaPerkawinan_niksaksi2
            ,'tglpemberkatan'   => $data->aktaPerkawinan_tglpemberkatan
            ,'tmptpemberkatan'   => $data->aktaPerkawinan_tmptpemberkatan
            ,'catatan'      => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'filekksuami'       => $data->syaratDok_KKSuami
            ,'filektpsuami' => $data->syaratDok_KTPSuami
            ,'filekkistri'  => $data->syaratDok_KKIstri
            ,'filektpistri' => $data->syaratDok_KTPIstri
            ,'filesuamiistri'   => $data->syaratDok_FotoSuamiIstri
            ,'filebukunikahagama'  => $data->syaratDok_BukuNikahAgama
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = PerkawinanModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => $data->pemohon_nama
            ...
            ,'tmptpemberkatan'   => $data->aktaPerkawinan_tmptpemberkatan
            ,'catatan'      => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'filekksuami'       => ($data->syaratDok_KKSuami)?'ADA':'-'
            ,'filektpsuami' => ($data->syaratDok_KTPSuami)?'ADA':'-'
            ,'filekkistri'  => ($data->syaratDok_KKIstri)?'ADA':'-'
            ,'filektpistri' => ($data->syaratDok_KTPIstri)?'ADA':'-'
            ,'filesuamiistri'   => ($data->syaratDok_FotoSuamiIstri)?'ADA':'-'
            ,'filebukunikahagama'  => ($data->syaratDok_BukuNikahAgama)?'ADA':'-'
        ...
        $pdf = \PDF::loadView('fronts.permohonans.aktanikahPDF', $output);
