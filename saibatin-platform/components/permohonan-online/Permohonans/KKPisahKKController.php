...
class KKPisahKKController extends Controller
{   
    ...
    {   
        $pelayananID    = '5'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.kkpisahkk', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            /* setup 1 [start] */
            $permohonanType = 'kkpisahkk';
            $permohonanKet  = 'Pisah KK';
            $permohonanInitial  = 'KKP01';
            /* setup 1 [end] */
            ...
            $jenispisah = $request->get('jenispisah');
            $nikygpisah = $request->get('nikygpisah');
            $alasanpisah= $request->get('alasanpisah');
            $nikpasangan= $request->get('nikpasangan');
            $kecamatantujuan= $request->get('kecamatantujuan');
            $kelurahantujuan= $request->get('kelurahantujuan');
            $norwtujuan = $request->get('norwtujuan');
            $norttujuan = $request->get('norttujuan');
            $alamattujuan   = $request->get('alamattujuan');
            $filekkx= $request->get('filekkx');
            $filekkpasanganx= $request->get('filekkpasanganx');
            $filebukunikahx = $request->get('filebukunikahx');
            $filesuratceraix= $request->get('filesuratceraix');
            $fileaktamatix  = $request->get('fileaktamatix');
            ...
            if($act=='ins'):
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'jenispisah'    => 'required',
                        'nikygpisah'    => 'required',
                        'alasanpisah'   => 'required',
                        'nikpasangan'   => ($alasanpisah==1 || $alasanpisah==2 || $alasanpisah==5 || $alasanpisah==6 || $alasanpisah==7 || $alasanpisah==8)?'required':'',
                        'kecamatantujuan'   => ($alasanpisah==2 || $alasanpisah==4 || $alasanpisah==6 || $alasanpisah==8 || $alasanpisah==10)?'required':'',
                        'kelurahantujuan'   => ($alasanpisah==2 || $alasanpisah==4 || $alasanpisah==6 || $alasanpisah==8 || $alasanpisah==10)?'required':'',
                        'norwtujuan'    => ($alasanpisah==2 || $alasanpisah==4 || $alasanpisah==6 || $alasanpisah==8 || $alasanpisah==10)?'required':'',
                        'norttujuan'    => ($alasanpisah==2 || $alasanpisah==4 || $alasanpisah==6 || $alasanpisah==8 || $alasanpisah==10)?'required':'',
                        'alamattujuan'  => ($alasanpisah==2 || $alasanpisah==4 || $alasanpisah==6 || $alasanpisah==8 || $alasanpisah==10)?'required':'',
                        'filekkx'   => 'required',
                        'filekkpasanganx'   => ($alasanpisah==1 || $alasanpisah==2 || $alasanpisah==7 || $alasanpisah==8)?'required':'',
                        'filebukunikahx'    => ($alasanpisah==1 || $alasanpisah==2 || $alasanpisah==7 || $alasanpisah==8)?'required':'',
                        'filesuratceraix'   => ($alasanpisah==5 || $alasanpisah==6 || $alasanpisah==5 || $alasanpisah==6 || $alasanpisah==7 || $alasanpisah==8)?'required':'',
                        'fileaktamatix' => ($alasanpisah==9 || $alasanpisah==10)?'required':''
                    ]
                    ,[
                        'required'  => ':attribute'
                    ]
                ...
                if($validation->fails()):
                    ...
                    if($sessnik):
                        ...
                        if($act == "ins"):
                            try {
                                ...
                                $dataprepcv = new KKPisahKKModel([
                                    ...
                                    'kk_jenispisah' => $jenispisah,
                                    'kk_nikygpisah' => $nikygpisah,
                                    'kk_alasanpisah'    => $alasanpisah,
                                    'kk_nikpasangan'    => $nikpasangan,
                                    'kk_kecamatantujuan'=> $kecamatantujuan,
                                    'kk_kelurahantujuan'=> $kelurahantujuan,
                                    'kk_norwtujuan' => $norwtujuan,
                                    'kk_norttujuan' => $norttujuan,
                                    'kk_alamattujuan'   => $alamattujuan,
                                    'catatan_detail'=> $catatan,
                                    'syaratDok_KK'  => isset($filekkx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkx:'',
                                    'syaratDok_KKPasangan'  => isset($filekkpasanganx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkpasanganx:'',
                                    'syaratDok_BukuNikah'  => isset($filebukunikahx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filebukunikahx:'',
                                    'syaratDok_SuratCerai'  => isset($filesuratceraix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filesuratceraix:'',
                                    'syaratDok_AktaMati'  => isset($fileaktamatix)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileaktamatix:'',
                                ...
                                #whereIn('progress_status', ['2','3'])->where([['pemohon_kk','=',$pemohonkk],['kk_jenispisah','=',$jenispisah],['kk_nikygpisah','=',$nikygpisah],['status','=','1']]);
                                $isExistQuery= KKPisahKKModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filekkx','filekkpasanganx','filebukunikahx','filesuratceraix','fileaktamatix'];
                                        ...
                                        $PermohonanData     = KKPisahKKModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = KKPisahKKModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new KKPisahKKModel;
                                        ...
                                        $datamaskingMass= KKPisahKKModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KKPisahKKModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        $output = array(
            ...
            ,'jenispisah'   => $data->kk_jenispisah
            ,'nikygpisah'   => $data->kk_nikygpisah
            ,'alasanpisah'  => $data->kk_alasanpisah
            ,'nikpasangan'  => $data->kk_nikpasangan
            ,'kecamatantujuan'  => $data->kk_kecamatantujuan
            ,'kelurahantujuan'  => $data->kk_kelurahantujuan
            ,'norwtujuan'   => $data->kk_norwtujuan
            ,'norttujuan'   => $data->kk_norttujuan
            ,'alamattujuan' => $data->kk_alamattujuan
            ,'filekk'       => $data->syaratDok_KK
            ,'filekkpasangan'   => $data->syaratDok_KKPasangan
            ,'filebukunikah'=> $data->syaratDok_BukuNikah
            ,'filesuratcerai'   => $data->syaratDok_SuratCerai
            ,'fileaktamati' => $data->syaratDok_AktaMati
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KKPisahKKModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $data_kkPisahJenis  = OptionModel::where([['option_val','=',$data->kk_jenispisah],['option_group','=','kkPisahJenis'],['status','=','1']])->get()->first();
        $data_kkPisahAlasan = OptionModel::where([['option_val','=',$data->kk_alasanpisah],['option_group','=','kkPisahAlasan'],['status','=','1']])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => $data->pemohon_nama
            ...
            ,'pemohonhp'    => $data->pemohon_hp
            ,'pemohonemail' => $data->pemohon_email
            ,'jenispisahnm'     => $data->kk_jenispisah
            ,'jenispisahval'    => $data_kkPisahJenis->option_name
            ,'nikygpisah'   => $data->kk_nikygpisah
            ,'alasanpisahnm'    => $data->kk_alasanpisah
            ,'alasanpisahval'   => $data_kkPisahAlasan->option_name
            ,'nikpasangan'  => $data->kk_nikpasangan
            ,'kecamatantujuan'  => $data->kk_kecamatantujuan
            ,'kelurahantujuan'  => $data->kk_kelurahantujuan
            ,'norwtujuan'   => $data->kk_norwtujuan
            ...
            ,'filekkpasangan'   => ($data->syaratDok_KKPasangan)?'ADA':'-'
            ,'filebukunikah'=> ($data->syaratDok_BukuNikah)?'ADA':'-'
            ,'filesuratcerai'   => ($data->syaratDok_SuratCerai)?'ADA':'-'
            ,'fileaktamati' => ($data->syaratDok_AktaMati)?'ADA':'-'
            ,'catatan'      => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'prgsts'=> $data->progress_status
            ,'rjkalasan'=> $data->reject_alasan
            ,'rjkdetil' => $data->alasan_detail
        ...
        $pdf = \PDF::loadView('fronts.permohonans.kkpisahkkPDF', $output);
