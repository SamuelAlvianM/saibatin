...
interface FormData {
  ...
  // KK Numpang data
  kklama: string;
  kkygditempati: string;
  nikygnumpangkk: string;
  alasannumpangkk: string;
  // Files
  filekklamax: string;
  filekkygditempatix: string;
...
export default function KKNumpangModal({
  open,
  onOpenChange,
  mode = "create",
  permohonanId,
  ...
  const [uploadedFiles, setUploadedFiles] = useState<{
    kklama?: UploadedFile;
    kkygditempati?: UploadedFile;
    pendukung?: UploadedFile;
  }>({});

  const [touchedFields, setTouchedFields] = useState<Set<string>>(new Set());
  const [fieldErrors, setFieldErrors] = useState<Record<string, string>>({});

  // Select options from PHP OptionModel (kkNumpangAlasan)
  const alasanNumpangOptions = [
    { value: "1", label: "Pekerjaan" },
    { value: "2", label: "Pendidikan" },
    { value: "3", label: "Perawatan Kesehatan" },
  ...
  const validateField = (field: keyof FormData, value: string): string | null => {
    switch (field) {
      ...
      case "nikygnumpangkk":
        return validateNIK(value, "NIK Yang Numpang KK");
      case "pemohonkk":
        return validateKK(value, "Nomor KK Pemohon");
      case "kklama":
        return validateKK(value, "Nomor KK Lama");
      case "kkygditempati":
        return validateKK(value, "Nomor KK Yang Ditempati");
      case "pemohonhp":
        return validatePhone(value);
      case "pemohonnama":
        return validateRequired(value, "Nama Pemohon");
      case "alasannumpangkk":
        return validateRequired(value, "Alasan Numpang KK");
      case "filekklamax":
        return validateRequired(value, "Upload KK Lama");
      case "filekkygditempatix":
        return validateRequired(value, "Upload KK Yang Ditempati");
      default:
        return null;
    }
  };
  ...
  const validateStep = (step: number): { isValid: boolean; errors: string[] } => {
    ...
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        fieldsToValidate = ["kklama", "kkygditempati", "nikygnumpangkk", "alasannumpangkk"];
        break;
      case 3:
        fieldsToValidate = ["filekklamax", "filekkygditempatix"];
        break;
    }
  ...
  const fetchPermohonanData = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/kk-numpang/fetchDatas", {
        method: "POST",
        headers: {
      ...
      if (data) {
        setFormData({
          ...
          kklama: data.kklama || "",
          kkygditempati: data.kkygditempati || "",
          nikygnumpangkk: data.nikygnumpangkk || "",
          alasannumpangkk: data.alasannumpangkk || "",
          filekklamax: data.filekklama || "",
          filekkygditempatix: data.filekkygditempati || "",
  ...
  const handleFileUpload = async (
    ...
    fileType: "kklama" | "kkygditempati" | "pendukung"
  ) => {
    ...
    try {
      ...
      const fieldNameMap = {
        kklama: "filekklama",
        kkygditempati: "filekkygditempati",
        pendukung: "filependukung1",
      };
      ...
      const response = await fetch("/api/kk-numpang/upload", {
        method: "POST",
        body: formDataUpload,
      });
      ...
      if (result.error && result.error.length > 0) {
        ...
        const fieldMap = {
          kklama: "filekklamax",
          kkygditempati: "filekkygditempatix",
          pendukung: "filependukung1x",
        };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const endpoint = "/api/kk-numpang/postdata";
      const payload = {
        ...formData,
  ...
  const getStepProgress = () => {
    const steps = [
      ...
      ["kklama", "kkygditempati", "nikygnumpangkk", "alasannumpangkk"],
      ["filekklamax", "filekkygditempatix"],
    ];
  ...
  const steps = [
    ...
    { step: 2, label: "Data KK Numpang", icon: Home },
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-hidden flex flex-col w-full">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 px-1">
          <div className="space-y-4 py-4">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300">
                  ...
                  <Input
                    id="kklama"
                    value={formData.kklama}
                    onChange={(e) => handleInputChange("kklama", e.target.value)}
                    placeholder="Masukkan 16 digit KK Lama"
                    maxLength={16}
                    className={getInputClass("kklama")}
                  />
                  {showFieldError("kklama")}
                </div>
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-75">
                  ...
                  <Input
                    id="kkygditempati"
                    value={formData.kkygditempati}
                    onChange={(e) => handleInputChange("kkygditempati", e.target.value)}
                    placeholder="Masukkan 16 digit KK Yang Ditempati"
                    maxLength={16}
                    className={getInputClass("kkygditempati")}
                  />
                  {showFieldError("kkygditempati")}
                </div>
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-100">
                  ...
                  <Input
                    id="nikygnumpangkk"
                    value={formData.nikygnumpangkk}
                    onChange={(e) => handleInputChange("nikygnumpangkk", e.target.value)}
                    placeholder="Masukkan 16 digit NIK Yang Numpang KK"
                    maxLength={16}
                    className={getInputClass("nikygnumpangkk")}
                  />
                  {showFieldError("nikygnumpangkk")}
                </div>

                {/* Alasan Numpang KK */}
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-150">
                  <Label htmlFor="alasannumpangkk">Alasan Numpang KK *</Label>
                  <Select 
                    value={formData.alasannumpangkk} 
                    onValueChange={(value) => handleInputChange("alasannumpangkk", value)}
                  >
                    <SelectTrigger className={getSelectClass("alasannumpangkk")}>
                      <SelectValue placeholder="Pilih alasan numpang KK" />
                    </SelectTrigger>
                    <SelectContent>
                      {alasanNumpangOptions.map((option) => (
                        <SelectItem key={option.value} value={option.value}>{option.label}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {showFieldError("alasannumpangkk")}
                </div>
            ...
            >
              {/* KK Lama Upload - Required */}
              <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300">
                <Label htmlFor="filekklama" className="flex items-center gap-2">
              ...
              {/* KK Yang Ditempati Upload - Required */}
              <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-75">
                <Label htmlFor="filekkygditempati" className="flex items-center gap-2">
                  <FileText className="h-4 w-4" />
                  Upload KK Yang Ditempati *
                  <span className="text-xs text-muted-foreground">(PNG/JPG/JPEG, Max 2MB)</span>
                </Label>
              ...
              <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-100">
                <Label htmlFor="filependukung" className="flex items-center gap-2">
                  <FileText className="h-4 w-4" />
