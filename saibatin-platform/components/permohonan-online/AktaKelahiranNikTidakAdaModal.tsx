...
export default function AktaKelahiranNikTidakAdaModal({
  open,
  onOpenChange,
  mode = "create",
  permohonanId,
}: AktaKelahiranNikTidakAdaModalProps) {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<{
    filebukunikah?: UploadedFile;
    filekk?: UploadedFile;
    fileketeranganlahir?: UploadedFile;
    filektpsaksi1?: UploadedFile;
    filektpsaksi2?: UploadedFile;
    filektpayah?: UploadedFile;
    filektpibu?: UploadedFile;
    filependukung1?: UploadedFile;
    filependukung2?: UploadedFile;
    filependukung3?: UploadedFile;
    filependukung4?: UploadedFile;
    filependukung5?: UploadedFile;
  }>({});
  ...
  const validateField = (
    ...
    switch (field) {
      ...
      case "namalengkap":
        return validateRequired(value, "Nama Lengkap Anak");
      case "jeniskelamin":
  ...
  const validateStep = (step: number): { isValid: boolean; errors: string[] } => {
    ...
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        fieldsToValidate = ["namalengkap", "jeniskelamin", "tgllahir"];
        break;
      case 3:
  ...
  const fetchPermohonanData = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/akta-kelahiran-nik-tidak-ada/fetchDatas", {
        method: "POST",
        headers: {
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      formDataUpload.append("typeset", fieldName);
      formDataUpload.append("uid", Date.now().toString());
      formDataUpload.append("groupset", fileType);

      const response = await fetch("/api/akta-kelahiran-nik-tidak-ada/upload", {
        method: "POST",
        body: formDataUpload,
      });
      ...
      if (result.error && result.error.length > 0) {
        ...
        handleInputChange(formField, serverFileName);
  ...
  const removeFile = (fileType: string, fieldName: string) => {
    const formField = `${fieldName}x` as keyof FormData;
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const endpoint = "/api/akta-kelahiran-nik-tidak-ada/postdata";
  ...
  const getStepProgress = () => {
    const steps = [
      ...
      ["namalengkap", "jeniskelamin", "tgllahir"],
  ...
  const steps = [
    ...
    { step: 3, label: "Data Orang Tua", icon: Users },
    { step: 4, label: "Data Kelahiran", icon: FileCheck },
    { step: 5, label: "Data Saksi", icon: Users },
  ...
  const renderFileUpload = (
    ...
    <div className={`space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 ${delay}`}>
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-hidden flex flex-col w-full">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 px-1">
          <div className="space-y-4 py-4">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2 md:col-span-2 animate-in slide-in-from-top-2 fade-in duration-300">
                  <Label htmlFor="namalengkap">Nama Lengkap Anak *</Label>
                  <Input
                    id="namalengkap"
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-75">
                  ...
                  >
                    <SelectTrigger className={getInputClass("tempatdilahirkan")}>
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-200">
                  ...
                  >
                    <SelectTrigger className={getInputClass("jeniskelahiran")}>
                ...
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-700">
                  ...
                  >
                    <SelectTrigger className={getInputClass("penolong")}>
            ...
            >
              {renderFileUpload("filebukunikah", "Upload Buku Nikah", "filebukunikah", "filebukunikah", true, "")}
              {renderFileUpload("filekk", "Upload Kartu Keluarga", "filekk", "filekk", true, "delay-75")}
              {renderFileUpload("fileketeranganlahir", "Upload Keterangan Lahir", "fileketeranganlahir", "fileketeranganlahir", true, "delay-100")}
              {renderFileUpload("filektpsaksi1", "Upload KTP Saksi 1", "filektpsaksi1", "filektpsaksi1", true, "delay-150")}
              {renderFileUpload("filektpsaksi2", "Upload KTP Saksi 2", "filektpsaksi2", "filektpsaksi2", true, "delay-200")}
              {renderFileUpload("filektpayah", "Upload KTP Ayah (Opsional)", "filektpayah", "filektpayah", false, "delay-300")}
              {renderFileUpload("filektpibu", "Upload KTP Ibu (Opsional)", "filektpibu", "filektpibu", false, "delay-500")}
              {renderFileUpload("filependukung1", "Upload Dokumen Pendukung 1 (Opsional)", "filependukung1", "filependukung1", false, "delay-700")}
              {renderFileUpload("filependukung2", "Upload Dokumen Pendukung 2 (Opsional)", "filependukung2", "filependukung2", false, "delay-1000")}
              {renderFileUpload("filependukung3", "Upload Dokumen Pendukung 3 (Opsional)", "filependukung3", "filependukung3", false, "delay-1000")}
              {renderFileUpload("filependukung4", "Upload Dokumen Pendukung 4 (Opsional)", "filependukung4", "filependukung4", false, "delay-1000")}
              {renderFileUpload("filependukung5", "Upload Dokumen Pendukung 5 (Opsional)", "filependukung5", "filependukung5", false, "delay-1000")}
