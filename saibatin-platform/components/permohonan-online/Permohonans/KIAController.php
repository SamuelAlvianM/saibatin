...
{   
    ...
    {   
        $pelayananID    = '12'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.kia', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }

    public function validatePick($key,$pickType) {
        if($pickType=='start'):
            $data = KKCetakUlangModel::find($key);
            $data->pick_by  = '';
            $data->pick_at  = '';
        elseif($pickType=='end'):
            $data = KKCetakUlangModel::find($key);
    ...
    {   
        ...
        if($recaptchaRes->isSuccess()):
            ...
            /* setup 1 [start] */
            $permohonanType = 'kia';
            $permohonanKet  = 'Kartu Identitas Anak (KIA)';
            $permohonanInitial  = 'KIA01';
            /* setup 1 [end] */
            ...
            $fileaktakelahiranx = $request->get('fileaktakelahiranx');
            $filepassfotox= $request->get('filepassfotox');
            ...
            if($act=='ins'):
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'fileaktakelahiranx'   => 'required',
                        'filekkx'   => 'required',
                        'filepassfotox'  => 'required',
                    ]
                    ,[
                        'required' 	=> ':attribute'
                    ]
    	            ...
    	            if($sessnik):
    	                ...
    	                if($act == "ins"):
    	                	try {
                                ...
                                $anakTglLahir   = Carbon::parse($tgllahir);
                                $anakUmUr   = $anakTglLahir->diffInYears(Carbon::now());

                                try {
                                    if($anakUmUr>17):
                                        $output_error[] = 'Info: Data tidak berhasil disimpan, Umur sudah diatas 16 Tahun. Saat ini umur yang diajukan permohonan KIA sudah '.$anakUmUr.' Tahun (N-08)';
                                    else:
                                        $dataprepcv = new KIAModel([
                                            ...
                                            'biodata_Kk'    => $pemohonkk,
                                            'biodata_anakNik'=> $nikanak,
                                            'biodata_anakNama'  => $namalengkap,
                                            'biodata_anakJenisKelamin'  => $jeniskelamin,
                                            'biodata_anakTanggalLahir'  => $tgllahir,
                                            'biodata_anakTempatLahir'   => $tempatlahir,
                                            'biodata_ayahNik'   => $nikayah,
                                            'biodata_ayahNama'  => $namaayah,
                                            'biodata_ibuNik'    => $nikibu,
                                            'biodata_ibuNama'   => $namaibu,
                                            'catatan_detail'=> $catatan,
                                            'syaratDok_aktaKelahiran'   => isset($fileaktakelahiranx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileaktakelahiranx:'',
                                            'syaratDok_PassFoto'    => isset($filepassfotox)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filepassfotox:'',
                                        ...
                                        $isExistQuery= KIAModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                        if($isExistQuery->count()<=0):
                                            ...
                                            if($savedata):
                                                ...
                                                $fileList  = ['fileaktakelahiranx','filekkx','filepassfotox','filependukung1x','filependukung2x'];
                                                ...
                                                $PermohonanData     = KIAModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                                    ...
                                    $output_error[]         = 'Info: Data tidak berhasil disimpan (N-07)';
                                }
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = KIAModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new KIAModel;
                                        ...
                                        $datamaskingMass= KIAModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KIAModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        ...
        $output = array(
            ...
            ,'nikanak'  => $data->biodata_anakNik
            ,'namalengkap'  => $data->biodata_anakNama
            ,'tgllahir' => $data->biodata_anakTanggalLahir
            ,'tempatlahir'  => $data->biodata_anakTempatLahir
            ,'jeniskelamin' => $data->biodata_anakJenisKelamin
            ,'nikayah'  => $data->biodata_ayahNik
            ,'namaayah' => $data->biodata_ayahNama
            ,'nikibu'   => $data->biodata_ibuNik
            ,'namaibu'  => $data->biodata_ibuNama

            ,'catatan'  => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'fileaktakelahiran'=> $data->syaratDok_aktaKelahiran
            ,'filekk'   => $data->syaratDok_KK
            ,'filepassfoto'     => $data->syaratDok_PassFoto
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KIAModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        ...
        $output = array(
            ...
            ,'pemohonemail' => $data->pemohon_email
            ,'nikanak'  => $data->biodata_anakNik
            ,'namalengkap'  => $data->biodata_anakNama #$getdatas->namalengkap
            ,'tgllahir' => $data->biodata_anakTanggalLahir #$getdatas->tgllhr
            ,'tempatlahir'  => $data->biodata_anakTempatLahir #$getdatas->tmptlahir
            ,'jeniskelamin' => $data_jenkel->option_name #$getdatas->jenkel
            ,'nikayah'  => $data->biodata_ayahNik #$getdatas->ayahnama
            ,'namaayah' => $data->biodata_ayahNama #$getdatas->ayahnama
            ,'nikibu'   => $data->biodata_ibuNik #$getdatas->ayahnama
            ,'namaibu'  => $data->biodata_ibuNama #$getdatas->ibunama
            ,'catatan'  => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'fileaktakelahiran'=> ($data->syaratDok_aktaKelahiran)?'ADA':'-'
            ,'filekk'   => ($data->syaratDok_KK)?'ADA':'-'
            ,'filepassfoto'     => ($data->syaratDok_PassFoto)?'ADA':'-'
        ...
        $pdf = \PDF::loadView('fronts.permohonans.kiaPDF', $output);
