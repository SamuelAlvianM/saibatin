...
use App\Models\Fronts\Users\UserlevelModel;
...
class KKCetakUlangController extends Controller
{   
    ...
    {   
        $pelayananID    = '8'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.kkcetakulang', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }

    public function validatePick($nomorPermohonan) {
        $data = KKCetakUlangModel::find($nomorPermohonan);
        $data->pick_by  = '';
        $data->pick_at  = '';
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            /* setup 1 [start] */
            $permohonanType = 'kkcetakulang';
    		$permohonanKet  = 'Cetak Ulang KK';
            $permohonanInitial  = 'KKC01';
            /* setup 1 [end] */
            ...
            $alasancetakulang   = $request->get('alasancetakulang');
            $alamatkepalakeluarga   = $request->get('alamatkepalakeluarga');

            $filekkx= $request->get('filekkx');
            $filesuratkehilanganx   = $request->get('filesuratkehilanganx');
            ...
            if($act=='ins'):
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'alasancetakulang'      => 'required',
                        'alamatkepalakeluarga'  => 'required',

                        'filekkx'   => ($alasancetakulang!='1')?'required':'',
                        'filesuratkehilanganx'  => ($alasancetakulang=='1')?'required':''
                    ]
                    ,[
                        'required'  => ':attribute'
                    ]
                    ...
                    if($sessnik):
                        if($act == "ins"):
                            try {
                                ...
                                $dataprepcv = new KKCetakUlangModel([
                                    ...
                                    'kk_namakepalakeluarga' => $namakepalakeluarga,
                                    'kk_alasancetakulang'   => $alasancetakulang,
                                    'kk_alamatkepalakeluarga'   => $alamatkepalakeluarga,
                                    'catatan_detail'=> $catatan,
                                    'syaratDok_KK'  => isset($filekkx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkx:'',
                                    'syaratDok_SuratKehilangan' => isset($filesuratkehilanganx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filesuratkehilanganx:'',
                                ...
                                $isExistQuery= KKCetakUlangModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filekkx','filesuratkehilanganx'];
                                        ...
                                        $PermohonanData         = KKCetakUlangModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = KKCetakUlangModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new KKCetakUlangModel;
                                        $ListFieldOfTable_get       = $ListFieldOfTable_init->getTableColumns();
                                        $ListFieldOfTable_remove    = ['id','nomorPermohonan','progress_status','isEscalateToDinas','catatan_admin','reject_alasan','alasan_detail','evidenceDelete_status','evidenceDelete_by','evidenceDelete_date','skm_key','status','kode_kel','created_by','created_at','updated_by','updated_at'];
                                        ...
                                        $datamaskingMass= KKCetakUlangModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KKCetakUlangModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        ...
        $output = array(
            ...
            ,'namakepalakeluarga'   => $data->kk_namakepalakeluarga
            ,'alasancetakulang'     => $data->kk_alasancetakulang
            ,'alamatkepalakeluarga' => $data->kk_alamatkepalakeluarga
            ,'filekk'   => $data->syaratDok_KK
            ,'filesuratkehilangan'   => $data->syaratDok_SuratKehilangan
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KKCetakUlangModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $data_alasancetakulang = OptionModel::where([['option_val','=',$data->kk_alasancetakulang],['option_group','=','alasancetakulang'],['status','=','1']])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => $data->pemohon_nama
            ...
            ,'pemohonemail' => $data->pemohon_email
            ,'namakepalakeluarga'   => $data->kk_namakepalakeluarga
            ,'alasancetakulangnm'   => $data->kk_alasancetakulang
            ,'alasancetakulangval'  => $data_alasancetakulang->option_name
            ,'alamatkepalakeluarga' => $data->kk_alamatkepalakeluarga
            ,'filekk'=> ($data->syaratDok_KK)?'ADA':'-'
            ,'filesuratkehilangan'  => ($data->syaratDok_SuratKehilangan)?'ADA':'-'
            ,'catatan'  => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'prgsts'=> $data->progress_status
            ,'rjkalasan'=> $data->reject_alasan
            ,'rjkdetil' => $data->alasan_detail
        ...
        $pdf = \PDF::loadView('fronts.permohonans.kkcetakulangPDF', $output);
