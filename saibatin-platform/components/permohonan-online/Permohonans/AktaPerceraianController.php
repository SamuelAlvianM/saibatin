...
class AktaPerceraianController extends Controller
{   
    ...
    {   
        $pelayananID    = '9'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.aktaperceraian', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            /* setup 1 [start] */
            $permohonanType = 'perceraian';
            $permohonanKet  = 'Akta Perceraian';
            $permohonanInitial  = 'AKC01';
            /* setup 1 [end] */
            ...
            $yangmengajukan     = $request->get('yangmengajukan');
            $alasancerai= $request->get('alasancerai');
            $noputusanpengadilan= $request->get('noputusanpengadilan');
            $tglputusan = $request->get('tglputusan');
            $instansipemberiputusan     = $request->get('instansipemberiputusan');
            $filekkx= $request->get('filekkx');
            $filektpsuamix  = $request->get('filektpsuamix');
            $filektpistrix  = $request->get('filektpistrix');
            $fileputusanx   = $request->get('fileputusanx');
            ...
            if($act=='ins'):
                ...
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'alasancerai' => 'required',
                        'noputusanpengadilan'    => 'required',
                        'tglputusan'  => 'required',
                        'instansipemberiputusan'   => 'required',
                        
                        'filekkx' => 'required',
                        'filektpsuamix'    => 'required',
                        'filektpistrix'   => 'required',
                        'fileputusanx'   => 'required'
                    ]
                    ,[
                        'required'  => ':attribute'
                    ]
                    ...
                    if($sessnik):
                        if($act == "ins"):
                            try {
                                ...
                                $dataprepcv = new PerceraianModel([
                                    ...
                                    'dataPerceraian_nokk'=> $nokk,
                                    'dataPerceraian_niksuami'   => $niksuami,
                                    'dataPerceraian_nikistri'   => $nikistri,
                                    'dataPerceraian_ygmengajukan'   => $yangmengajukan,
                                    'dataPerceraian_alasancerai'    => $alasancerai,
                                    'dataPerceraian_noputusanpengadilan'    => $noputusanpengadilan,
                                    'dataPerceraian_tglputusan'  => $tglputusan,
                                    'dataPerceraian_instansipemberiputusan' => $instansipemberiputusan,
                                    ...
                                    'syaratDok_FilePutusan' => isset($fileputusanx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileputusanx:'',
                                ...
                                $isExistQuery= PerceraianModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filekkx','filektpsuamix','filektpistrix','fileputusanx','filependukung1x','filependukung2x'];
                                        ...
                                        $PermohonanData     = PerceraianModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = PerceraianModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new PerceraianModel;
                                        ...
                                        $datamaskingMass= PerceraianModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = PerceraianModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        ...
        $output = array(
            ...
            ,'nokk'     => $data->dataPerceraian_nokk
            ,'niksuami' => $data->dataPerceraian_niksuami
            ,'nikistri' => $data->dataPerceraian_nikistri
            ,'yangmengajukan' => $data->dataPerceraian_ygmengajukan
            ,'alasancerai'  => $data->dataPerceraian_alasancerai
            ,'noputusanpengadilan'      => $data->dataPerceraian_noputusanpengadilan
            ,'tglputusan'   => $data->dataPerceraian_tglputusan
            ,'instansipemberiputusan'   => $data->dataPerceraian_instansipemberiputusan
            ...
            ,'fileputusan'  => $data->syaratDok_FilePutusan
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = PerceraianModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $data_aktaPerceraianYgMengajukan    = OptionModel::where([['option_val','=',$data->dataPerceraian_ygmengajukan],['option_group','=','aktaPerceraianYgMengajukan'],['status','=','1']])->get()->first();
        $data_aktaPerceraianAlasan  = OptionModel::where([['option_val','=',$data->dataPerceraian_alasancerai],['option_group','=','aktaPerceraianAlasan'],['status','=','1']])->get()->first();
        $data_aktaPerceraianInstansiPemberiKeputusan    = OptionModel::where([['option_val','=',$data->dataPerceraian_instansipemberiputusan],['option_group','=','aktaPerceraianInstansiPemberiKeputusan'],['status','=','1']])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => $data->pemohon_nama
            ...
            ,'niksuami' => $data->dataPerceraian_niksuami
            ,'nikistri' => $data->dataPerceraian_nikistri
            ,'yangmengajukan' => $data_aktaPerceraianYgMengajukan->option_name
            ,'alasancerai'  => $data_aktaPerceraianAlasan->option_name
            ,'noputusanpengadilan'      => $data->dataPerceraian_noputusanpengadilan
            ,'tglputusan'   => $data->dataPerceraian_tglputusan
            ,'instansipemberiputusan'   => $data_aktaPerceraianInstansiPemberiKeputusan->option_name
            ,'catatan'  => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'filekk'   => ($data->syaratDok_KK)?'ADA':''
            ,'filektpsuami' => ($data->syaratDok_KTPSuami)?'ADA':''
            ,'filektpistri' => ($data->syaratDok_KTPIstri)?'ADA':''
            ,'fileputusan'  => ($data->syaratDok_FilePutusan)?'ADA':''
        ...
        $pdf = \PDF::loadView('fronts.permohonans.aktaperceraianPDF', $output);
