...
class KKPerubahanBiodataController extends Controller
{   
    ...
    {   
        $pelayananID    = '4'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.kkperubahanbiodata', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            $kk     = $request->get('kk');
            $nik    = $request->get('nik');
            $jenisbiodata   = preg_replace('/\s+/', '', $request->get('jenisbiodata'));
            ...
            $fileaktalahirx = $request->get('fileaktalahirx');
            $fileijazahx= $request->get('fileijazahx');
            ...
            if($act=='ins'):
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'namalengkap'   => (str_contains($jenisbiodata,'namalengkap'))?'required':'',
                        'jeniskelamin'  => (str_contains($jenisbiodata,'jeniskelamin'))?'required':'',
                        'tempatlahir'   => (str_contains($jenisbiodata,'tempatlahir'))?'required':'',
                        'tanggallahir'  => (str_contains($jenisbiodata,'tanggallahir'))?'required':'',
                        'golongandarah' => (str_contains($jenisbiodata,'golongandarah'))?'required':'',
                        'agama'     => (str_contains($jenisbiodata,'agama'))?'required':'',
                        'pendidikan'=> (str_contains($jenisbiodata,'pendidikan'))?'required':'',
                        'pekerjaan' => (str_contains($jenisbiodata,'pekerjaan'))?'required':'',
                        'namaayah'  => (str_contains($jenisbiodata,'namaayah'))?'required':'',
                        'namaibu'   => (str_contains($jenisbiodata,'namaibu'))?'required':'',
                        'statusperkawinan'  => (str_contains($jenisbiodata,'statusperkawinan'))?'required':'',

                        'filekkx'   => 'required',
                        'fileaktalahirx'    => (preg_match('(namalengkap|jeniskelamin|tempatlahir|tanggallahir|namaayah|namaibu)',$jenisbiodata) === 1)?'required':'',
                        'fileijazahx'       => (preg_match('(namalengkap|tempatlahir|tanggallahir|pendidikan)',$jenisbiodata) === 1)?'required':'',
                        'filebukunikahx'    => (str_contains($jenisbiodata,'statusperkawinan'))?'required':''
                    ]
                    ,[
                        'required'  => ':attribute'
                    ]
                    ...
                    if($sessnik):
                        ...
                        if($act == "ins"):
                            try {
                                ...
                                $lastSeqQuery   = DB::query()
                                    ...
                                    ->fromSub(
                                        function ($query) use($permohonanInitial,$genNoPermohonanTimestamp){
                                            $query
                                                ->from('t_kk_perubahanbiodata as x1')
                                                ->selectRaw('
                                ...
                                $dataprepcv = new KKPerubahhanBiodataModel([
                                    ...
                                    'kk_kk'     => $kk,
                                    'kk_nik'    => $nik,
                                    'kk_jenisbiodata'   => $jenisbiodata,
                                    'kk_nama'   => $namalengkap,
                                    'kk_jeniskelamin'=> $jeniskelamin,
                                    'kk_tempatlahir' => $tempatlahir,
                                    'kk_tanggallahir'=> $tanggallahir,
                                    'kk_golongandarah'  => $golongandarah,
                                    'kk_agama'  => $agama,
                                    'kk_pendidikan' => $pendidikan,
                                    'kk_pekerjaan'  => $pekerjaan,
                                    'kk_namaayah'   => $namaayah,
                                    'kk_namaibu'    => $namaibu,
                                    'kk_statusperkawinan'   => $statusperkawinan,
                                    'catatan_detail'=> $catatan,
                                    'syaratDok_KK'  => isset($filekkx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkx:'',
                                    'syaratDok_AktaLahir'   => isset($fileaktalahirx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileaktalahirx:'',
                                    'syaratDok_Ijazah'      => isset($fileijazahx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileijazahx:'',
                                ...
                                #whereIn('progress_status', ['2','3'])->where([['pemohon_kk','=',$pemohonkk],['kk_kk','=',$kk],['kk_nik','=',$nik],['status','=','1']]);
                                $isExistQuery= KKPerubahhanBiodataModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filekkx','fileaktalahirx','fileijazahx','filebukunikahx','filependukung1x','filependukung2x','filependukung3x','filependukung4x'];
                                        ...
                                        $PermohonanData     = KKPerubahhanBiodataModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = KKPerubahhanBiodataModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new KKPerubahhanBiodataModel;
                                        ...
                                        $datamaskingMass= KKPerubahhanBiodataModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KKPerubahhanBiodataModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        $output = array(
            ...
            ,'kk'   => $data->kk_kk
            ,'nik'  => $data->kk_nik
            ,'jenisbiodata' => $data->kk_jenisbiodata
            ,'namalengkap' => $data->kk_nama
            ,'jeniskelamin' => $data->kk_jeniskelamin
            ,'tempatlahir'  => $data->kk_tempatlahir
            ,'tanggallahir' => $data->kk_tanggallahir
            ,'golongandarah'=> $data->kk_golongandarah
            ,'agama'=> $data->kk_agama
            ,'pendidikan'   => $data->kk_pendidikan
            ,'pekerjaan'=> $data->kk_pekerjaan
            ,'namaayah' => $data->kk_namaayah
            ,'namaibu'  => $data->kk_namaibu
            ,'statusperkawinan'  => $data->kk_statusperkawinan
            ,'filekk'   => $data->syaratDok_KK
            ,'fileaktalahir'=> $data->syaratDok_AktaLahir
            ,'fileijazah'   => $data->syaratDok_Ijazah
    ...
    {   
        $result = DB::table('m_options as a1')
                    ->selectRaw('a1.option_val as opt_val, a1.option_name as opt_name, a1.sort as opt_sort')
                    ->where([
                        ['a1.status', '=', 1]
                        , ['a1.option_group', '=', 'jenisPerubahanBiodata']
                    ])
                    ->get()
                    ->toJson();
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KKPerubahhanBiodataModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $data_jenkel    = OptionModel::where([['option_val','=',$data->kk_jeniskelamin],['option_group','=','jenkel'],['status','=','1']])->get()->first();
        $data_agama     = OptionModel::where([['option_val','=',$data->kk_agama],['option_group','=','agama'],['status','=','1']])->get()->first();
        $data_pekerjaan = OptionModel::where([['option_val','=',$data->kk_pekerjaan],['option_group','=','pekerjaan'],['status','=','1']])->get()->first();
        $data_golongandarah = OptionModel::where([['option_val','=',$data->kk_golongandarah],['option_group','=','golongandarah'],['status','=','1']])->get()->first();
        $data_pendidikan= OptionModel::where([['option_val','=',$data->kk_pendidikan],['option_group','=','pendidikan'],['status','=','1']])->get()->first();
        $data_statusperkawinan  = OptionModel::where([['option_val','=',$data->kk_statusperkawinan],['option_group','=','statusperkawinan'],['status','=','1']])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => $data->pemohon_nama
            ...
            ,'jenisbiodata' => $data->kk_jenisbiodata
            ,'namalengkap'  => $data->kk_nama
            ,'jeniskelaminnm'   => $data->kk_jeniskelamin
            ,'jeniskelaminval'  => ($data_jenkel)?$data_jenkel->option_name:''
            ,'tempatlahir'  => $data->kk_tempatlahir
            ,'tanggallahir' => $data->kk_tanggallahir
            ,'golongandarahnm'  => $data->kk_golongandarah
            ,'golongandarahval' => ($data_golongandarah)?$data_golongandarah->option_name:''
            ,'agamanm'  => $data->kk_agama
            ,'agamaval' => ($data_agama)?$data_agama->option_name:''
            ,'pendidikannm' => $data->kk_pendidikan
            ,'pendidikanval'=> ($data_pendidikan)?$data_pendidikan->option_name:''
            ,'pekerjaannm'  => $data->kk_pekerjaan
            ,'pekerjaanval' => ($data_pekerjaan)?$data_pekerjaan->option_name:''
            ,'namaayah' => $data->kk_namaayah
            ,'namaibu'  => $data->kk_namaibu
            ,'statusperkawinannm' => $data->kk_statusperkawinan
            ,'statusperkawinanval'=> ($data_statusperkawinan)?$data_statusperkawinan->option_name:''
            ,'filekk'   => ($data->syaratDok_KK)?'ADA':'-'
            ,'fileaktalahir'=> ($data->syaratDok_AktaLahir)?'ADA':'-'
            ,'fileijazah'   => ($data->syaratDok_Ijazah)?'ADA':'-'
        ...
        $pdf = \PDF::loadView('fronts.permohonans.kkperubahanbiodataPDF', $output);
