...
interface FormData {
  ...
  alasancetakulang: string;
  alamatkepalakeluarga: string;
  // Files
  filekkx: string;
...
export default function KKCetakUlangModal({
  open,
  onOpenChange,
  mode = "create",
  permohonanId,
}: KKCetakUlangModalProps) {
  ...
  // Alasan cetak ulang options from PHP
  const alasanCetakUlangOptions = [
    { value: "1", label: "Hilang" },
    { value: "2", label: "Rusak" },
    { value: "3", label: "Data Berubah" },
    { value: "4", label: "Karena Adanya Perubahan Data" },
  ...
  const validateField = (
    ...
    switch (field) {
      ...
      case "namakepalakeluarga":
        return validateRequired(value, "Nama Kepala Keluarga");
      case "alasancetakulang":
        return validateRequired(value, "Alasan Cetak Ulang");
      case "alamatkepalakeluarga":
        return validateRequired(value, "Alamat Kepala Keluarga");
      case "filekkx":
        if (showKKFile()) {
  ...
  const validateStep = (step: number): { isValid: boolean; errors: string[] } => {
    ...
    switch (step) {
      case 1:
        ...
        break;
      case 2:
        fieldsToValidate = ["namakepalakeluarga", "alasancetakulang", "alamatkepalakeluarga"];
        break;
      case 3:
        fieldsToValidate = [];
        if (showKKFile()) {
          fieldsToValidate.push("filekkx");
        }
        if (showSuratKehilangan()) {
  ...
  // Helper to check if surat kehilangan is required
  const showSuratKehilangan = () => {
    return formData.alasancetakulang === "1"; // Only for "Hilang"
  };

  // Helper to check if KK file is required
  const showKKFile = () => {
    return formData.alasancetakulang !== "1" && formData.alasancetakulang !== ""; // Required for all except "Hilang" and when not selected
  };
  ...
  const fetchPermohonanData = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/kk-cetak-ulang/fetch", {
        method: "POST",
        headers: {
      ...
      if (data) {
        setFormData({
          ...
          alasancetakulang: data.alasancetakulang || "",
          alamatkepalakeluarga: data.alamatkepalakeluarga || "",
          filekkx: data.filekk || "",
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      const response = await fetch("/api/kk-cetak-ulang/upload", {
        method: "POST",
        body: formDataUpload,
      });
  ...
  const removeFile = (fileType: "kk" | "suratkehilangan") => {
    const fieldMap = {
      kk: "filekkx",
      suratkehilangan: "filesuratkehilanganx",
    };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const endpoint =
        mode === "edit"
          ? "/api/kk-cetak-ulang/update"
          : "/api/kk-cetak-ulang/create";
  ...
  const getStepProgress = () => {
    const steps = [
      ...
      ["namakepalakeluarga", "alasancetakulang", "alamatkepalakeluarga"],
      showKKFile() ? ["filekkx"] : showSuratKehilangan() ? ["filesuratkehilanganx"] : [],
    ];
  ...
  const steps = [
    ...
    { step: 2, label: "Data KK", icon: Home },
    { step: 3, label: "Dokumen", icon: Printer },
  ];
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-hidden flex flex-col w-full">
        <DialogHeader className="space-y-1 pb-1 border-b">
          <DialogTitle className="text-2xl font-bold bg-linear-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">
            {mode === "edit" ? "Edit Permohonan" : "Permohonan Cetak Ulang KK"}
          </DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground">
            Lengkapi formulir cetak ulang kartu keluarga dengan data yang akurat
          </DialogDescription>
          ...
          <div className="space-y-4 py-4">
              ...
              <div className="flex justify-end pt-4 gap-4">
                <div className="flex items-center gap-2">
                  <div className="h-2 w-32 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-linear-to-r from-blue-500 to-cyan-500 transition-all duration-500" style={{ width: `${progress.step1}%` }} />
                  </div>
                ...
                <Button
                  ...
                  className="bg-linear-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 transition-all duration-300"
                >
                  Selanjutnya
                </Button>
              </div>
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300">
                  ...
                  <Input
                    ...
                    className={getInputClass("namakepalakeluarga")}
                  />
                  {showFieldError("namakepalakeluarga")}
                </div>

                <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-75">
                  <Label htmlFor="alasancetakulang">Alasan Cetak Ulang *</Label>
                  <Select
                    value={formData.alasancetakulang}
                    onValueChange={(value) => {
                      handleInputChange("alasancetakulang", value);
                      // Reset files when alasan changes
                      if (value === "1") {
                        handleInputChange("filekkx", "");
                        setUploadedFiles((prev) => ({ ...prev, kk: undefined }));
                      } else {
                        handleInputChange("filesuratkehilanganx", "");
                        setUploadedFiles((prev) => ({ ...prev, suratkehilangan: undefined }));
                      }
                    }}
                  >
                    <SelectTrigger className={getSelectClass("alasancetakulang")}>
                      <SelectValue placeholder="Pilih alasan cetak ulang" />
                    </SelectTrigger>
                    <SelectContent>
                      {alasanCetakUlangOptions.map((option) => (
                  ...
                  {showFieldError("alasancetakulang")}
                </div>

                <div className="space-y-2 md:col-span-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-150">
                  <Label htmlFor="alamatkepalakeluarga">Alamat Kepala Keluarga *</Label>
                  <Textarea
                    id="alamatkepalakeluarga"
                    value={formData.alamatkepalakeluarga}
                    onChange={(e) => handleInputChange("alamatkepalakeluarga", e.target.value)}
                    placeholder="Masukkan alamat lengkap kepala keluarga"
                    rows={3}
                    className={getInputClass("alamatkepalakeluarga")}
                  />
                  {showFieldError("alamatkepalakeluarga")}
                </div>
              ...
              <div className="flex justify-between pt-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-32 bg-gray-200 rounded-full overflow-hidden">
                      <div className="h-full bg-linear-to-r from-blue-500 to-cyan-500 transition-all duration-500" style={{ width: `${progress.step2}%` }} />
                    </div>
            ...
            >
              <div className="grid grid-cols-1 gap-4">
                {/* KK Upload - Conditional Required */}
                {showKKFile() && (
                  <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300">
                ...
                {/* Surat Kehilangan Upload - Conditional Required */}
                {showSuratKehilangan() && (
                  <div className="space-y-2 animate-in slide-in-from-top-2 fade-in duration-300 delay-75">
                      ...
                      <div className="relative group">
                        <div className="flex items-center justify-between p-4 border-2 border-green-500 rounded-lg bg-linear-to-r from-green-50 to-emerald-50 transition-all duration-300 hover:shadow-md">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium text-green-900">{uploadedFiles.suratkehilangan.name}</p>
                              <p className="text-xs text-green-600">Berhasil diupload</p>
                            </div>
                          </div>
                          <Button
              ...
              <div className="flex justify-between pt-4">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    <div className="h-2 w-32 bg-gray-200 rounded-full overflow-hidden">
                      <div className="h-full bg-linear-to-r from-blue-500 to-cyan-500 transition-all duration-500" style={{ width: `${progress.step3}%` }} />
                    </div>
                  ...
                  <Button
                    type="submit"
                    disabled={loading}
                    className="bg-linear-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 transition-all duration-300 shadow-lg hover:shadow-xl"
                  >
                    {loading ? (
                      <>
