...
{   
    ...
    {   
        $pelayananID    = '14'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.kedatangan', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            /* setup 1 [start] */
            $permohonanType = 'kedatangan';
            $permohonanKet  = 'Kedatangan Penduduk';
            $permohonanInitial  = 'PKD01';
            /* setup 1 [end] */
            ...
            $skpwni = $request->get('skpwni');
            $namaPemohon= $request->get('namapemohon');
            $nikPemohon = $request->get('nikpemohon');
            $nohp   = $request->get('nohp');
            $email  = $request->get('email');
            $kecamatanTujuan= $request->get('kecamatantujuan');
            $desaTujuan = $request->get('desatujuan');
            $dusunTujuan= $request->get('dusuntujuan');
            $filesuratpindahx = $request->get('filesuratpindahx');
            ...
            if($act=='ins'):
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'nohp'  => 'required',
                        'email' => 'required',
                        'kecamatantujuan'   => 'required',
                        'desatujuan'    => 'required',
                        'dusuntujuan'   => 'required',
                        'filesuratpindahx' => 'required'
                    ]
                    ,[
                        'required' 	=> ':attribute'
                    ]
                    ...
                    if($act == "ins"):
                        ...
                        if($nikPemohon):
    	                	try {
                                ...
                                $dataprepcv = new KedatanganModel([
                                    ...
                                    'biodata_Nik'   => $pemohonnik,
                                    'biodata_Kk'    => $pemohonkk,
                                    'dataKedatangan_skpwni'=> $skpwni,
                                    'dataKedatangan_namaPemohon'=> $namaPemohon,
                                    'dataKedatangan_nikPemohon' => $nikPemohon,
                                    'dataKedatangan_noHp'   => $nohp,
                                    'dataKedatangan_email'  => $email,
                                    'dataKedatangan_kecamatanTujuan'    => $kecamatanTujuan,
                                    'dataKedatangan_desaTujuan' => $desaTujuan,
                                    'dataKedatangan_dusunTujuan'=> $dusunTujuan,
                                    'dataKedatangan_catatan'=> $catatan,
                                    'syaratDok_suratPindah'   => isset($filesuratpindahx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filesuratpindahx:'',
                                ...
                                $isExistQuery= KedatanganModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filesuratpindahx','filebukunikahx','filependukung1x','filependukung2x'];
                                        ...
                                        $PermohonanData     = KedatanganModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                            ...
                            $output_error[] = 'Info: Harap input NIK Pemohon (N-01)';
                        endif;
                    elseif($request->get('act') == "upd"):
                        try {
                            $data = KedatanganModel::find($request->get('key'));
                            ...
                            if($updatedata):
                                ...
                                if($prgsts=='0'):
                                    $ListFieldOfTable_init      = new KedatanganModel;
                                    ...
                                    $datamaskingMass= KedatanganModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KedatanganModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        $output = array(
            ...
            ,'skpwni'   => $data->dataKedatangan_skpwni
            ,'namapemohon' => $data->dataKedatangan_namaPemohon
            ,'nikpemohon' => $data->dataKedatangan_nikPemohon
            ,'nohp' => $data->dataKedatangan_noHp
            ,'email'  => $data->dataKedatangan_email
            ,'kecamatantujuan' => $data->dataKedatangan_kecamatanTujuan
            ,'desatujuan'=> $data->dataKedatangan_desaTujuan
            ,'dusuntujuan'   => $data->dataKedatangan_dusunTujuan
            ,'catatan'  => ($data->dataKedatangan_catatan)?$data->dataKedatangan_catatan:'-'
            ,'filesuratpindah'  => $data->syaratDok_suratPindah
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KedatanganModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $output = array(
            ...
            ,'dusuntujuan'   => $data->dataKedatangan_dusunTujuan
            ,'catatan'  => ($data->dataKedatangan_catatan)?$data->dataKedatangan_catatan:'-'
            ,'filesuratpindah'  => ($data->syaratDok_suratPindah)?'ADA':''
        ...
        $pdf = \PDF::loadView('fronts.permohonans.kedatanganPDF', $output);
