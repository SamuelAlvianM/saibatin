...
class KTPELController extends Controller
{   
    ...
    {   
        $pelayananID    = '15'; /*m_setup_pelayanan*/
        ...
        if(!$data_OperasionalWaktu):
            ...
            return view('fronts.permohonans.ktpel', ['id'=>$urlRoute,'key'=>$key,'modal'=>$modal,'prm1'=>$prm1,'prm2'=>$prm2,'prm3'=>$prm3]);
        endif;
    }
    ...
    public function postdata(Request $request)
    {
        ...
        if($recaptchaRes->isSuccess()):
            ...
            /* setup 1 [start] */
            $permohonanType = 'ktpel';
            $permohonanKet  = 'KTP-El';
            $permohonanInitial  = 'KTP01';
            /* setup 1 [end] */
            ...
            $alasancetak    = $request->get('alasancetak');
            
            $filekkx= $request->get('filekkx');
            $filektplamax  = $request->get('filektplamax');
            $filesuratkehilanganx   = $request->get('filesuratkehilanganx');
            $fileketeranganx  = $request->get('fileketeranganx');
            ...
            if($act=='ins'):
                $validation = Validator::make(
                    ...
                    ,[
                        ...
                        'nik'   => 'required',
                        'nama'  => 'required',
                        'alasancetak'   => 'required',
                        
                        'filekkx'       => 'required',
                        'filektplamax'  => ($alasancetak==1 || $alasancetak==6)?'required':'',
                        'filesuratkehilanganx'   => ($alasancetak==2)?'required':'',
                        'fileketeranganx'   => ($alasancetak==3)?'required':''
                    ]
                    ,[
                        'required'  => ':attribute'
                    ]
                ...
                if($validation->fails()):
                    ...
                    if($sessnik):
                        ...
                        if($act == "ins"):
                            try {
                                ...
                                $dataprepcv = new KTPELModel([
                                    ...
                                    'ktpel_nik'     => $nik,
                                    'ktpel_nama'    => $nama,
                                    'ktpel_alasancetak' => $alasancetak,
                                    'catatan_detail'=> $catatan,
                                    'syaratDok_KK'  => isset($filekkx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filekkx:'',
                                    'syaratDok_KTPLama' => isset($filektplamax)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filektplamax:'',
                                    'syaratDok_SuratKehilangan' => isset($filesuratkehilanganx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$filesuratkehilanganx:'',
                                    'syaratDok_Keterangan'  => isset($fileketeranganx)?'uploads/'.$permohonanType.'/'.$genNoPermohonan.'/'.$fileketeranganx:'',
                                ...
                                $isExistQuery= KTPELModel::whereIn('progress_status', ['2','3'])->where([['pemohon_nik','=',$pemohonnik],['status','=','1']]);
                                if($isExistQuery->count()<=0):
                                    ...
                                    if($savedata):
                                        ...
                                        $fileList  = ['filekkx','filektplamax','filesuratkehilanganx','fileketeranganx'];
                                        ...
                                        $PermohonanData     = KTPELModel::where([['nomorPermohonan',$genNoPermohonan]])->get()->first();
                        ...
                        elseif($request->get('act') == "upd"):
                            try {
                                $data = KTPELModel::find($request->get('key'));
                                ...
                                if($updatedata):
                                    ...
                                    if($prgsts=='0'):
                                        $ListFieldOfTable_init      = new KTPELModel;
                                        ...
                                        $datamaskingMass= KTPELModel::find($request->get('key'));
    ...
    {   
        ...
        $data   = KTPELModel::where([['nomorPermohonan','=',$nomorPermohonan]])->get()->first();
        ...
        $output = array(
            ...
            ,'nokk'         => $data->ktpel_nokk
            ,'nik'          => $data->ktpel_nik
            ,'nama'         => $data->ktpel_nama
            ,'alasancetak'  => $data->ktpel_alasancetak
            ,'catatan'      => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'filekk'       => $data->syaratDok_KK
            ,'filektplama' => $data->syaratDok_KTPLama
            ,'filesuratkehilangan'  => $data->syaratDok_SuratKehilangan
            ,'fileketerangan' => $data->syaratDok_Keterangan
    ...
    public function genPDF($permohonanType,$permohonanID) {
        $data   = KTPELModel::where([['nomorPermohonan','=',$permohonanID]])->get()->first();
        $data_ktpElAlasan = OptionModel::where([['option_val','=',$data->ktpel_alasancetak],['option_group','=','ktpElAlasan'],['status','=','1']])->get()->first();
        $output = array(
            'permohonanid'  => $data->nomorPermohonan
            ,'pemohonnik'   => $data->pemohon_nik
            ,'pemohonnama'  => $data->pemohon_nama
            ...
            ,'nik'          => $data->ktpel_nik
            ,'nama'         => $data->ktpel_nama
            ,'alasancetaknm'=> $data->ktpel_alasancetak
            ,'alasancetakval'   => $data_ktpElAlasan->option_name
            ,'catatan'      => ($data->catatan_detail)?$data->catatan_detail:'-'
            ,'filekk'       => ($data->syaratDok_KK)?'ADA':''
            ,'filektplama'  => ($data->syaratDok_KTPLama)?'ADA':''
            ,'filesuratkehilangan'  => ($data->syaratDok_SuratKehilangan)?'ADA':''
            ,'fileketerangan'   => ($data->syaratDok_Keterangan)?'ADA':''
            ,'prgsts'=> $data->progress_status
            ,'rjkalasan'=> $data->reject_alasan
            ,'rjkdetil' => $data->alasan_detail
            ,'sts'   => $data->status
        ...
        $pdf = \PDF::loadView('fronts.permohonans.ktpelPDF', $output);
